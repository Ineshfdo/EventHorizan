...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $event_date = $_POST['date']; // From datetime-local input
    ...
    $currentDateTime = date('Y-m-d H:i'); // Current date and time in same format
    if ($event_date <= $currentDateTime) {
        $error = "You cannot add an event in the past.";
    } else {
        ...
        if ($count > 0) {
            $error = "An event already exists at the selected date and time.";
        } else {
            ...
            $sql = "INSERT INTO events 
                    (title, description, event_date, main_image, extra_image_1, extra_image_2, extra_image_3, club_id, created_at)
                    VALUES (:title, :description, :event_date, :main_image, :img1, :img2, :img3, :club_id, NOW())";
...
$clubQuery = "SELECT id, club_name FROM clubs";

...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $event_date = $_POST['date']; // datetime-local input
    ...
    $stmt = $pdo->prepare("UPDATE events SET title=?, description=?, event_date=?, main_image=?, extra_image_1=?, extra_image_2=?, extra_image_3=?, club_id=? WHERE id=?");
    $stmt->execute([$title, $description, $event_date, $main_image, $img1, $img2, $img3, $club_id, $event_id]);
...
<aside class="hidden md:flex flex-col w-64 p-6 bg-black text-white flex-shrink-0">
  ...
  <nav class="flex flex-col gap-4 flex-1">
    <a href="dashboard.php" class="flex items-center gap-3 p-2 hover:bg-gray-800 rounded-lg">Dashboard</a>
    <a href="club_add.php" class="flex items-center gap-3 p-2 hover:bg-gray-800 rounded-lg">Add Club</a>
    <a href="manage_clubs.php" class="flex items-center gap-3 p-2 hover:bg-gray-800 rounded-lg">Manage Clubs</a>
    <a href="event_add.php" class="flex items-center gap-3 p-2 hover:bg-gray-800 rounded-lg">Add Event</a>
    <a href="manage_events.php" class="flex items-center gap-3 p-2 bg-gray-800 rounded-lg">Manage Events</a>
    <a href="admin_logout.php" class="flex items-center gap-3 p-2 hover:bg-gray-800 rounded-lg mt-2">Logout</a>
  </nav>
</aside>
  ...
  <div class="max-w-3xl bg-white p-8 shadow-lg rounded-xl mx-auto">
    <form action="" method="POST" enctype="multipart/form-data" class="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="title" value="<?= htmlspecialchars($event['title']) ?>" class="w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" required>
      </div>
      ...
      <div>
        ...
        <input type="datetime-local" name="date" value="<?= date('Y-m-d\TH:i', strtotime($event['event_date'])) ?>" class="w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" required>
      </div>
      ...
      <div>
        ...
        <select name="club_id" class="w-full border p-2 rounded focus:outline-none focus:ring-2 focus:ring-blue-500" required>
          <?php foreach($clubs as $club): ?>
            <option value="<?= $club['id'] ?>" <?= $club['id'] == $event['club_id'] ? 'selected' : '' ?>><?= htmlspecialchars($club['club_name']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
